...
setup() {
	...
	load "common"
	server_net1=$(get_net1_ip6 "test-simple" "pod-server")
	client_a_net1=$(get_net1_ip6 "test-simple" "pod-client-a")
	client_b_net1=$(get_net1_ip6 "test-simple" "pod-client-b")
}

@test "setup simple test environments" {
	skip
	# create test manifests
	kubectl create -f simple-v6.yml

	# verify all pods are available
	run kubectl -n test-simple wait --for=condition=ready -l app=test-simple pod --timeout=${kubewait_timeout}
	[ "$status" -eq  "0" ]
}

@test "check generated ip6tables rules" {
	# check pod-server has multi-networkpolicy iptables rules for ingress
        run kubectl -n test-simple exec pod-server -- sh -c "ip6tables-save | grep MULTI-0-INGRESS"
	[ "$status" -eq  "0" ]
	# check pod-client-a has NO multi-networkpolicy iptables rules for ingress
        run kubectl -n test-simple exec pod-client-a -- sh -c "ip6tables-save | grep MULTI-0-INGRESS"
	[ "$status" -eq  "1" ]
	# check pod-client-b has NO multi-networkpolicy iptables rules for ingress
        run kubectl -n test-simple exec pod-client-b -- sh -c "ip6tables-save | grep MULTI-0-INGRESS"
	[ "$status" -eq  "1" ]
...
@test "test-simple check client-a -> server" {
	# nc should succeed from client-a to server by policy
	run kubectl -n test-simple exec pod-client-a -- sh -c "echo x | nc -w 1 ${server_net1} 5555"
	[ "$status" -eq  "0" ]
}

@test "test-simple check client-b -> server" {
	# nc should NOT succeed from client-b to server by policy
	run kubectl -n test-simple exec pod-client-b -- sh -c "echo x | nc -w 1 ${server_net1} 5555"
	[ "$status" -eq  "1" ]
}

@test "test-simple check server -> client-a" {
	# nc should succeed from server to client-a by no policy definition for direction (egress for pod-server)
	run kubectl -n test-simple exec pod-server -- sh -c "echo x | nc -w 1 ${client_a_net1} 5555"
	[ "$status" -eq  "0" ]
}

@test "test-simple check server -> client-b" {
	# nc should succeed from server to client-b by no policy definition for direction (egress for pod-server)
	run kubectl -n test-simple exec pod-server -- sh -c "echo x | nc -w 1 ${client_b_net1} 5555"
	[ "$status" -eq  "0" ]
}
...
@test "disable multi-networkpolicy and check iptables rules" {
	...
	# check iptable rules in pod-server
        run kubectl -n test-simple exec pod-server -it -- sh -c "iptables-save | grep MULTI-0-INGRESS"
	[ "$status" -eq  "1" ]
...
@test "cleanup environments" {
	...
	kubectl delete -f simple-v6.yml
	run kubectl -n test-simple wait --for=delete -l app=test-simple pod --timeout=${kubewait_timeout}
	[ "$status" -eq  "0" ]
