...
func NewServer(o *Options) (*Server, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("server creation failed for kubeconfig [%s] master[%s]: %w", o.Kubeconfig, o.master, err)
	}
...
func (s *Server) generatePolicyRules(pod *v1.Pod, podInfo *controllers.PodInfo) error {
	...
	for _, p := range s.policyMap {
		...
		if policy.Spec.PodSelector.Size() != 0 {
			...
			if err != nil {
				klog.Errorf("bad label selector for policy [%s]: %v", namespacedName(policy), err)
				continue
			}
	...
	if err := iptableBuffer.SyncRules(s.ip4Tables); err != nil {
		klog.Errorf("sync rules failed for pod [%s]: %v", namespacedName(pod), err)
		return err
	}
